...
export function GoogleLoginButton({ onSuccess, onError }: GoogleLoginButtonProps) {
  ...
  const handleGoogleLogin = async () => {
    ...
    setTimeout(() => {
      ...
      const mockGoogleUser = {
        ...
        name: 'John Doe',
        picture: undefined, // Could add a placeholder image URL
        given_name: 'John',
        family_name: 'Doe'
      };
    ...
    // Or use the new Google Identity Services
  };
  ...
  return (
    <button
      ...
      disabled={isLoading}
      className="w-full bg-white border border-gray-300 rounded-lg px-6 py-3 flex items-center justify-center gap-3 hover:bg-gray-50 hover:shadow-md transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
    >
      {isLoading ? (
        <>
